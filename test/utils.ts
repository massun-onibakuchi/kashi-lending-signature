...
export function getDomainSeparator(name: string, bentoBox: string, chainId: number) {
    return keccak256(
        defaultAbiCoder.encode(
            ["bytes32", "bytes32", "uint256", "address"],
            [EIP712_DOMAIN_TYPEHASH, keccak256(toUtf8Bytes(name)), chainId, bentoBox],
        ),
    );
}
...
export const getApproveData = (user: string, masterContract: string, approved: boolean, v, r, s) => {
